...
// import { useSearchParams, useRouter, usePathname } from 'next/navigation';
// import Navbar from '@/components/shared/Navbar';
// // ❌ We only import TYPES now, not the static data
...
//   RefreshCcw,
// } from 'lucide-react';
// import { Icon } from '@/components/shared/IconMapper'; // Import IconMapper
...
// // --- Reusable Filter Button ---
// // ✅ This component is fine, but we'll use <Icon> for consistency
// const FilterButton = ({
//   icon: IconComponent, // Renamed to avoid conflict with our Icon mapper
//   label,
//   onClick,
//   isActive,
// }: {
...
// // --- Search Header Component ---
// // ✅ Must accept 'developers' prop
// function SearchHeader({ developers }: { developers: Developer[] }) {
//   const router = useRouter();
//   const pathname = usePathname();
//   const searchParams = useSearchParams();
...
//     zoning: ['Residential', 'Commercial'],
//     // ✅ Use 'developers' prop
//     builders: [...new Set(developers.map((d) => d.name))].sort(),
//     propertyType: ['Apartments', 'Villa', 'Plot'],
//     status: ['Ready to Move', 'Under Construction', 'Sold Out', 'New Launch'],
//     bedrooms: ['1 BHK', '2 BHK', '3 BHK', '4 BHK', '5+ BHK'],
//   };

//   // ... (all other functions inside SearchHeader are fine)
//   // ... (useMemo, handleClickOutside, updateQueryParam, handleSearch, etc.)
...
// // ✅ Accept 'properties' and 'developers' as props
// export default function SearchResultsClient({ properties, developers }: SearchResultsClientProps) {
//   const searchParams = useSearchParams();
//   const [currentPage, setCurrentPage] = useState(1);
//   const ITEMS_PER_PAGE = 9;
...
//   const status = searchParams.get('status');
//   const bedrooms = searchParams.get('bedrooms');
//   const budget = parseInt(searchParams.get('budget') || '300000000', 10); // Use a realistic max default

// // Inside useMemo for filteredProperties:
// const filteredProperties = useMemo(() => {
//   // ✅ Use the 'properties' prop, not 'propertiesData'
//   return properties.filter((property) => {
//     // ✅ Use the 'developers' prop, not 'developersData'
...
//     // Property Type
//     // ✅ Check against 'specs' which is more reliable
//     const propertyTypeMatch = !propertyType || propertyType === 'Select' || 
//       (property.specs.some(s => s.icon === 'Building2' && s.text.toLowerCase().trim() === propertyType.toLowerCase().trim()));

//     // Status
//     const statusMatch = !status || status === 'Select' || 
//       (property.tag?.text.toLowerCase().trim() === status.toLowerCase().trim());

//     // Bedrooms: compare normalized (remove spaces)
//     const normalizeBedroom = (str: string) => str.toLowerCase().replace(/\s+/g, '');
//     const bedroomsMatch = !bedrooms || bedrooms === 'Select' || 
//       property.specs.some((s) => s.icon === 'BedDouble' && normalizeBedroom(s.text) === normalizeBedroom(bedrooms));

//     return textMatch && zoningMatch && builderMatch && propertyTypeMatch && statusMatch && bedroomsMatch;
//   });
// }, [query, city, zoning, builder, propertyType, status, bedrooms, budget, properties, developers]); // ✅ Add props to dependency array
...
//     <div className="bg-gray-50 min-h-screen">
//       <Navbar />
//       {/* ✅ Pass the 'developers' prop to SearchHeader */}
//       <SearchHeader developers={developers} />

//       <main className="container mx-auto max-w-7xl px-4 py-8">
//         <div className="flex justify-between items-center mb-6">
//           <p className="text-gray-600">
//             <strong>{filteredProperties.length}</strong> Results in{' '}
//   a         <strong>{city}</strong>
//             {query && (
//               <>
//                 {' '}
//                 for "<strong className="text-green-600">{query}</strong>"
...
//             currentPage={currentPage}
//             onPageChange={setCurrentPage}
//             // ✅ Pass 'developers' prop to PropertyListings
...
// --- Helper to get unique, sorted strings from property data ---
function getUniqueOptions(properties: Property[], getter: (p: Property) => string | undefined) {
...
// --- Search Header Component ---
function SearchHeader({ properties, developers }: { properties: Property[]; developers: Developer[] }) {
  const router = useRouter();
  const pathname = usePathname();
  const searchParams = useSearchParams();
  ...
  const activeFilters = {
    ...
    status: searchParams.get('status'),
    bedrooms: searchParams.get('bedrooms'),
    budget: searchParams.get('budget') || '300000000',
  };

  const filterOptions = useMemo(() => {
    return {
      zoning: getUniqueOptions(properties, (p) => p.overview.zoning),
      builders: [...new Set(developers.map((d) => d.name))].sort(),
      propertyType: getUniqueOptions(properties, (p) =>
        p.specs.find((s) => s.icon === 'Building2')?.text
      ),
      status: getUniqueOptions(properties, (p) => p.tag?.text),
      bedrooms: getUniqueOptions(properties, (p) =>
        p.specs.find((s) => s.icon === 'BedDouble')?.text
      ),
    };
  ...
  const areFiltersActive = useMemo(
    () =>
      ...
      searchParams.has('status') ||
      searchParams.has('bedrooms') ||
      (searchParams.get('budget') && searchParams.get('budget') !== '300000000'),
    [searchParams]
  );
  ...
  const formatBudget = (value: string) => {
    const num = parseInt(value, 10);
    if (num >= 300000000) return '₹ 30 Cr+';
    if (num >= 10000000) return `₹ ${(num / 10000000).toFixed(2)} Cr`;
    return `₹ ${(num / 100000).toFixed(0)} L`;
  };

  return (
    <div className="bg-white py-8 border-b border-gray-200 sticky top-0 z-20" ref={filterRef}>
      <div className="container mx-auto max-w-7xl px-4">
        <div className="mb-6">
          <h1 className="text-4xl font-serif text-gray-800 tracking-wider">SEARCH</h1>
          <p className="text-sm font-semibold text-green-600 tracking-[0.2em]">
        ...
        <div className="mt-6 flex flex-wrap items-center gap-x-6 gap-y-3 text-sm">
          ...
          <FilterButton
            ...
            label="Budget"
            onClick={() => toggleFilter('Budget')}
            isActive={activeFilters.budget !== '300000000'}
          />
          ...
          <div className="mt-4 bg-gray-100/70 p-6 rounded-lg border border-gray-200 absolute w-[calc(100%-2rem)] left-4 md:w-full md:left-auto md:relative shadow-lg md:shadow-none">
            {openFilter === 'Zoning' && (
              filterOptions.zoning.length > 0 ? (
                ...
                <p className="text-gray-500">No zoning options available.</p>
              )
            )}
            ...
            {openFilter === 'Builders' && (
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2">
                {filterOptions.builders.length > 0 ? (
                  ...
                  <p className="text-gray-500 col-span-full">No builders available.</p>
                )}
              </div>
            )}
            ...
            {openFilter === 'Property Type' && (
              filterOptions.propertyType.length > 0 ? (
                ...
                <p className="text-gray-500">No property types available.</p>
              )
            )}
            ...
            {openFilter === 'Bedrooms' && (
              filterOptions.bedrooms.length > 0 ? (
                ...
                <p className="text-gray-500">No bedroom options available.</p>
              )
            )}
            ...
            {openFilter === 'Budget' && (
              <div>
                <div className="flex justify-between items-center mb-2">
                  <label className="font-medium text-gray-700">Max Budget:</label>
                  <span className="font-bold text-green-600 text-lg">
                    {formatBudget(activeFilters.budget)}
                  </span>
                </div>
...
export default function SearchResultsClient({ properties, developers }: SearchResultsClientProps) {
  ...
  const status = searchParams.get('status');
  const bedrooms = searchParams.get('bedrooms');
  const budget = parseInt(searchParams.get('budget') || '300000000', 10);
  ...
  const filteredProperties = useMemo(() => {
    return properties.filter((property) => {
      ...
      // Zoning
      const zoningMatch = !zoning || property.overview.zoning?.toLowerCase().trim() === zoning.toLowerCase().trim();
      ...
      const propertyTypeMatch =
        !propertyType ||
        property.specs.some((s) => s.icon === 'Building2' && s.text === propertyType);

      // Status
      const statusMatch = !status || property.tag?.text === status;

      // Bedrooms
      const bedroomsMatch =
        !bedrooms || property.specs.some((s) => s.icon === 'BedDouble' && s.text === bedrooms);

      return textMatch && zoningMatch && builderMatch && propertyTypeMatch && statusMatch && bedroomsMatch;
    });
  }, [query, city, zoning, builder, propertyType, status, bedrooms, budget, properties, developers]);
  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <Navbar />
      <SearchHeader properties={properties} developers={developers} />
