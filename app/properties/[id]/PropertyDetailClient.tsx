...
interface PropertyDetailClientProps {
  property: SerializableProperty;
  developer?: Developer;
  relatedProperties: SerializableProperty[];
}
...
function PropertyHeader({
  ...
  const [isShareModalOpen, setIsShareModalOpen] = useState(false);
  const { addFavorite, removeFavorite, isFavorite } = useFavorites();
  const isFav = isFavorite(property.id); // ✅ id is string
  const propertyDetailsRef = useRef<HTMLDivElement>(null);
  const [isCapturing, setIsCapturing] = useState(false);
  const [shareUrl, setShareUrl] = useState('');
  ...
  const generateAndShareImage = async () => {
    ...
    try {
      ...
      if (navigator.share) {
        if (imageFile && navigator.canShare?.(shareData)) {
          await navigator.share(shareData);
        } else {
          await navigator.share({ title: shareTitle, text: shareText, url: shareUrl });
        }
  ...
  return (
    <>
      ...
      {/* ✅ bg-white ensures clean capture background */}
      <div ref={propertyDetailsRef} className="grid grid-cols-1 lg:grid-cols-5 gap-8 bg-white">
        <div className="lg:col-span-3">
          <div className="relative w-full h-[300px] md:h-[500px] rounded-xl overflow-hidden shadow-lg mb-4">
            <Image
...
function PropertyTabs({
  ...
  if (property.salientFeatures?.length > 0) availableTabs.push("Salient Features");
  if (Object.keys(property.specifications || {}).some(k => (property.specifications as any)[k]?.length > 0))
    availableTabs.push("Specifications");
  if (property.floorplans?.length > 0) availableTabs.push("Floorplans");
  if (property.images?.length > 1) availableTabs.push("Gallery"); // >1 because main image is already shown
  availableTabs.push("EMI Calculator");
  if (developer) availableTabs.push("About Us");
  if (property.amenities?.length > 0) availableTabs.push("Amenities");
  if (property.faq?.length > 0) availableTabs.push("FAQs");
    ...
    <div className="mt-16">
      ...
      <div className="py-8">
        ...
        {activeTab === "Floorplans" && (
          <FloorplansTabContent
            floorplans={property.floorplans || []}
            developerName={developer?.name || property.developer}
            location={property.location}
          />
        )}
        {activeTab === "Gallery" && (
          <GalleryTabContent images={property.images} propertyName={property.name} />
        )}
        ...
        {activeTab === "FAQs" && <FAQTabContent faqs={property.faq || []} />}
      </div>
    </div>
  );
}
...
// --- Tab Contents (Overview, Specs, etc.) ---
function OverviewTabContent({ property }: { property: SerializableProperty }) {
  return (
    <div>
      <div className="grid grid-cols-2 md:grid-cols-4 gap-8 text-sm">
...
function SalientFeaturesTabContent({ features }: { features: string[] }) {
  if (!features || features.length === 0) {
    return <p className="text-gray-500">No salient features listed.</p>;
  }
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
      {features.map((feature, index) => (
...
function SpecificationsTabContent({ specs }: { specs: Record<string, string[]> }) {
  const specCategories = Object.keys(specs).filter((key) => Array.isArray(specs[key]) && specs[key].length > 0);
  const [activeSpecTab, setActiveSpecTab] = useState(specCategories[0] || "");
  ...
  return (
    <div>
      <div className="flex flex-wrap items-center gap-2 border-b pb-2 mb-6">
        {specCategories.map((category) => (
          <button
            key={category}
            onClick={() => setActiveSpecTab(category)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
        {(specs[activeSpecTab] || []).map((spec, index) => (
          <FeatureListItem key={index} text={spec} />
        ))}
      </div>
    </div>
...
function FloorplansTabContent({
  ...
  if (!floorplans || floorplans.length === 0) {
    return <p className="text-gray-500">No floorplans available.</p>;
  }
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
      {floorplans.map((plan, index) => (
        <FloorplanCard key={index} plan={plan} developerName={developerName} location={location} />
      ))}
    </div>
  );
}
...
function GalleryTabContent({ images, propertyName }: { images: string[]; propertyName: string }) {
  if (!images || images.length <= 1) {
    return <p className="text-gray-500">No additional gallery images.</p>;
  }
  // Exclude first image (already in header)
...
function EMICalculatorTabContent({ property }: { property: SerializableProperty }) {
  ...
  const parsePrice = (priceStr: string): number => {
    const match = priceStr.match(/([\d.]+)\s*(Cr\.|L)/);
    if (match) {
      const value = parseFloat(match[1]);
      if (match[2] === "Cr.") return value * 10000000;
      if (match[2] === "L") return value * 100000;
    }
  ...
  const basePrice = property.price || (property.floorplans.length > 0 ? parsePrice(property.floorplans[0].priceRange) : 5000000);
  const [loanAmount, setLoanAmount] = useState(basePrice);

  useEffect(() => {
    if (property.floorplans?.length > 0) {
      const newLoanAmount = parsePrice(property.floorplans[selectedUnitIndex]?.priceRange || property.floorplans[0].priceRange);
      setLoanAmount(newLoanAmount);
    } else {
      setLoanAmount(basePrice);
    }
  }, [selectedUnitIndex, property.floorplans, basePrice]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
      <div className="space-y-6">
        {property.floorplans && property.floorplans.length > 0 ? (
          ...
          <p className="text-gray-500">EMI calculated based on base property price.</p>
        )}
      ...
      <div className="w-full">
        ...
        >
          ...
          <ChevronDown
            size={20}
            className={`text-green-600 transition-transform ${isChartVisible ? "rotate-180" : ""}`}
          />
        </button>
        {isChartVisible && (
          <div className="mt-6 flex flex-col items-center">
...
function RelatedProperties({
  ...
  if (!relatedProperties || relatedProperties.length === 0) return null;
  return (
    <div className="mt-8 py-8 border-t">
      ...
      <div className="text-center mt-12">
        <button  className="bg-white border-2 border-green-500 text-green-600 font-bold py-3 px-8 rounded-full hover:bg-green-50 transition">
           <Link href={`/search`} className="block">View More</Link>
        </button>
      </div>
    </div>
  );
...
function RelatedPropertyCard({ property }: { property: SerializableProperty }) {
  return (
    <Link href={`/properties/${property.id}`} className="block">
      <div className="bg-white rounded-lg shadow-md border border-gray-200 transition-shadow hover:shadow-xl overflow-hidden">
        <div className="relative w-full h-52">
        ...
        <div className="p-5">
          ...
          <hr className="border-dashed my-3" />
          <div className="flex justify-between items-center text-xs text-gray-700">
            {property.specs.slice(0, 3).map((spec, index) => (
              <div key={index} className="flex items-center gap-1">
...
function AmenitiesTabContent({ amenities }: { amenities: SerializableProperty["amenities"] }) {
  if (!amenities || amenities.length === 0) {
    return (
      <div className="text-center text-gray-500 py-10">
        No amenities listed for this property.
...
function FAQTabContent({ faqs }: { faqs: { question: string; answer: string }[] }) {
        ...
        <div key={index} className="border border-gray-200 rounded-lg overflow-hidden">
          ...
          {openIndex === index && (
            <motion.div
              ...
              className="px-5 pb-5 text-gray-600 leading-relaxed bg-gray-50"
            >
              {faq.answer}
            </motion.div>
          )}
