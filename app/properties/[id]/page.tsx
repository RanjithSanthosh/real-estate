...
// import { propertiesData, Property,developersData, Developer } from '../../data/properties';
// import Navbar from '@/components/shared/Navbar';
// import { Heart, MapPin, Phone, Share2, Printer, Car, Building2, BedDouble, Scaling } from 'lucide-react';
...
//           <Image src={activeImage} alt={property.name} fill className="object-cover" priority />
...
//             <div key={index} className={`relative h-24 rounded-lg overflow-hidden cursor-pointer border-2 ${activeImage === img ? 'border-green-500' : 'border-transparent'}`} onClick={() => setActiveImage(img)}>
...
//         <div className="flex items-center text-gray-600 mb-4"><MapPin size={16} className="mr-2 text-blue-500"/><span>{property.location}</span></div>
//         <div className="flex flex-wrap gap-2 mb-4">
//           {property.specs.map((spec, i) => (<div key={i} className="bg-green-50 text-green-800 text-sm font-medium px-4 py-2 rounded-full flex items-center gap-2 border border-green-200"><spec.icon size={16}/> {spec.text}</div>))}
//         </div>
//         <div className="bg-gray-50 rounded-lg p-4 my-4"><p className="text-gray-600 text-sm">Price</p><p className="text-3xl font-bold text-green-600">{property.priceRange}</p></div>
//         <div className=" sm:flex-row gap-4 mt-6">
//           <button className="flex-1 bg-white text-green-600 border-2 border-green-500 font-bold py-3 px-6 rounded-3xl hover:bg-green-50 transition w-lg">Download Brochure</button>
//           <button className="flex-1 bg-green-500 text-white font-bold py-3 px-6 rounded-3xl hover:bg-green-600 transition flex items-center justify-center gap-2 w-lg mt-5  "><Phone size={18}/> Call Us</button>
//         </div>
//       </div>
//     </div>
//   );
...
// //           {['Overview', 'Salient Features', 'Specifications', 'Floorplans', 'Gallery', 'EMI Calculator', 'About Us'].map(tab => (
// //             <button key={tab} onClick={() => setActiveTab(tab)} className={`whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm ${activeTab === tab ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
// //               {tab}
// //             </button>
// //           ))}
// //         </nav>
...
// // --- SUB-COMPONENT: PROPERTY TABS (Full Replacement) ---
// function PropertyTabs({ property }: { property: Property }) {
//   const [activeTab, setActiveTab] = useState('Overview');
...
//           {['Overview', 'Salient Features', 'Specifications', 'Floorplans', 'Gallery', 'EMI Calculator', 'About Us'].map(tab => (
//             <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm ${activeTab === tab ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
//               {tab}
//             </button>
//           ))}
//         </nav>
...
//         {activeTab === 'Salient Features' && <SalientFeaturesTabContent features={property.salientFeatures} />}
//         {activeTab === 'Specifications' && <SpecificationsTabContent specs={property.specifications} />}
...
//             />
//         )}
//         {activeTab === 'Gallery' && <GalleryTabContent images={property.images} propertyName={property.name} />}
//         {activeTab === 'EMI Calculator' && <EMICalculatorTabContent property={property} />}
//         {activeTab === 'About Us' && developer && <AboutUsTabContent developer={developer} />}
//       </div>
//     </div>
//   );
// }
...
// function SpecificationsTabContent({ specs }: { specs: Property['specifications'] }) {
...
//             className={`px-4 py-2 rounded-full text-sm font-semibold transition ${activeSpecTab === category ? 'bg-green-100 text-green-700' : 'text-gray-600 hover:bg-gray-100'}`}
//           >
//             {category}
//           </button>
//         ))}
...
// // function FloorplansTabContent({ floorplans }: { floorplans: Property['floorplans'] }) {
// //     return (
...
// //                         <div className="absolute top-3 left-3 bg-white/90 text-gray-800 text-sm font-bold px-3 py-1.5 rounded-full shadow-lg">{plan.priceRange}</div>
// //                          <div className="absolute top-3 right-3 flex gap-2">
// //                             <button className="w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg text-purple-600 hover:bg-gray-50"><Car size={20} /></button>
// //                             <button className="w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg text-pink-500 hover:bg-gray-50"><Heart size={20} /></button>
// //                         </div>
// //                     </div>
// //                     <div className="p-5">
...
// //                             {plan.specs.map((spec, i) => <span key={i} className="flex items-center gap-2 text-sm"><spec.icon size={16} />{spec.text}</span>)}
// //                         </div>
...
// //                             </button>
// //                             <button className="flex-grow bg-green-500 text-white py-4 px-6 rounded-full font-bold text-lg hover:bg-green-600 transition">Contact Us</button>
// //                         </div>
// //                     </div>
// //                 </div>
// //             ))}
...
// function FloorplansTabContent({ floorplans, developerName, location }: { 
//     floorplans: Property['floorplans'], 
...
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
//             {floorplans.map((plan, index) => (
//                 // ✅ We've created a dedicated card component for clarity
//                 <FloorplanCard 
//                     key={index} 
//                     plan={plan} 
...
// }) {
//     return (
//         <div className="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden transition-shadow hover:shadow-2xl">
//             {/* Image Section */}
//             <div className="relative w-full h-60">
//                 <Image src={plan.image} alt={plan.name} fill className="object-cover" />
...
// function GalleryTabContent({ images, propertyName }: { images: string[], propertyName: string }) {
//     return (
//         <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
//             {images.map((img, index) => (
//                 <div key={index} className="relative w-full h-48 rounded-lg overflow-hidden shadow-md">
//                     <Image src={img} alt={`${propertyName} gallery image ${index + 1}`} fill className="object-cover" />
//                 </div>
//             ))}
//         </div>
//     );
...
//                 <div><p className="text-4xl font-extrabold text-green-600">{developer.totalProjects}</p><p className="text-gray-500">Total Projects</p></div>
//                 <div><p className="text-4xl font-extrabold text-green-600">{developer.ongoingProjects}</p><p className="text-gray-500">Ongoing Projects</p></div>
//                 <div><p className="text-4xl font-extrabold text-green-600">{developer.yearsOfExperience}</p><p className="text-gray-500">Years of Experience</p></div>
//             </div>
//             <p className="text-gray-600 leading-relaxed max-w-3xl mx-auto">{developer.description}</p>
//             <div className="mt-6">
//                 <a href="#" className="text-green-600 font-semibold hover:underline">View more by builder</a>
//             </div>
//         </div>
//     );
// }
...
//         <Image src={property.mapImage} alt={`Map of ${property.name}`} fill className="object-cover" />
...
//     propertiesData.filter(p => p.developer === currentProperty.developer && p.id !== currentProperty.id)
...
//         {related.map(property => <RelatedPropertyCard key={property.id} property={property} />)}
//       </div>
...
//                     <div className="absolute top-3 left-3 bg-white/90 text-gray-800 text-sm font-bold px-3 py-1.5 rounded-full shadow-lg">{property.priceRange}</div>
//                     <div className="absolute top-3 right-3 flex gap-2">
//                         <button className="w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg text-purple-600 hover:bg-gray-50"><Car size={20} /></button>
//                         <button className="w-10 h-10 bg-white/90 rounded-full flex items-center justify-center shadow-lg text-pink-500 hover:bg-gray-50"><Heart size={20} /></button>
//                     </div>
...
//                     <div className="flex items-center text-gray-500 text-sm mb-4"><MapPin size={14} className="mr-1.5 flex-shrink-0" />{property.location}</div>
//                     <hr className="border-dashed my-3" />
//                     <div className="flex justify-between items-center text-xs text-gray-700">
//                         {property.specs.map((spec, index) => <span key={index}>{spec.text}</span>)}
//                     </div>
//                 </div>
//             </div>
//         </Link>
...
import { propertiesData, Property, developersData, Developer } from '../../data/properties';
import Navbar from '@/components/shared/Navbar';
import { Heart, MapPin, Phone, Share2, Printer, Car, Building2, BedDouble, Scaling, CheckCircle2, ChevronDown } from 'lucide-react';
...
// --- SUB-COMPONENT: PROPERTY TABS ---
function PropertyTabs({ property, developer }: { property: Property; developer?: Developer }) {
  const [activeTab, setActiveTab] = useState('Overview');
  ...
  return (
    <div className="mt-16">
      <div className="border-b border-gray-200">
        <nav className="-mb-px flex space-x-8 overflow-x-auto" aria-label="Tabs">
          {['Overview', 'Salient Features', 'Specifications', 'Floorplans', 'Gallery', 'EMI Calculator', 'About Us','Amenities'].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-shrink-0 whitespace-nowrap pb-4 px-1 border-b-2 font-medium text-sm ${activeTab === tab ? 'border-green-500 text-green-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}`}>
              {tab}
            </button>
          ))}
      ...
      <div className="py-8">
        ...
        {activeTab === 'EMI Calculator' && <EMICalculatorTabContent property={property} />}
        {activeTab === 'About Us' && developer && <AboutUsTabContent developer={developer} />}
        {activeTab === 'Amenities' && <AmenitiesTabContent amenities={property.amenities} />}
      </div>
    </div>
  );
}
        ...
        <div className="bg-white rounded-2xl shadow-lg border border-gray-100 overflow-hidden transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 flex flex-col">
            ...
            <div className="p-6 flex flex-col flex-grow">
                <div className="flex-grow">
                    <h3 className="text-xl font-extrabold text-gray-900 tracking-tight">{plan.name}</h3>
                    <p className="text-sm text-gray-500 mt-1">
                        by <a href="#" className="text-yellow-600 font-semibold underline">{developerName}</a>
                    </p>
                    <div className="flex items-center text-gray-500 text-sm mt-3">
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 items-start">
            ...
            <div className="space-y-6">
                {/* Unit Selector */}
                <div>
                    <label htmlFor="unit-select" className="block text-sm font-medium text-gray-700 mb-2">Select A Unit</label>
                ...
                <div className="space-y-8 pt-4">
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="text-sm font-medium text-gray-700">Load Amount</label>
                            <span className="font-bold text-green-600 text-sm">{formatCrores(loanAmount)}</span>
                        </div>
                        <input type="range" min="10000000" max="100000000" step="100000" value={loanAmount} onChange={(e) => setLoanAmount(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500" />
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>1Cr</span><span>2.5Cr</span><span>5Cr</span><span>7.5Cr</span><span>10Cr</span></div>
                    </div>
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="text-sm font-medium text-gray-700">Load Tenure</label>
                            <span className="font-bold text-green-600 text-sm">{loanTenure} yrs</span>
                        </div>
                        <input type="range" min="5" max="25" step="1" value={loanTenure} onChange={(e) => setLoanTenure(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500" />
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>5 yrs</span><span>10 yrs</span><span>15 yrs</span><span>20 yrs</span><span>25 yrs</span></div>
                    </div>
                    <div>
                        <div className="flex justify-between items-center mb-1">
                            <label className="text-sm font-medium text-gray-700">Interest Rate</label>
                            <span className="font-bold text-green-600 text-sm">{interestRate.toFixed(1)}% p.a</span>
                        </div>
                        <input type="range" min="6" max="12" step="0.1" value={interestRate} onChange={(e) => setInterestRate(Number(e.target.value))} className="w-full h-2 bg-gray-200 rounded-lg appearance-none cursor-pointer accent-green-500" />
                        <div className="flex justify-between text-xs text-gray-500 mt-1"><span>6%</span><span>9%</span><span>12%</span></div>
                    </div>
                </div>
            </div>
            ...
            <div className="w-full">
                <button onClick={() => setIsChartVisible(!isChartVisible)} className="flex items-center justify-between w-full text-left">
                    <span className="text-sm font-bold tracking-widest text-green-600">VIEW CHART</span>
                    <ChevronDown size={20} className={`text-green-600 transition-transform ${isChartVisible ? 'rotate-180' : ''}`} />
                </button>
                {isChartVisible && (
                    <div className="mt-6 flex flex-col items-center">
                        <div className="relative w-40 h-40">
                            <div className="absolute inset-0 rounded-full bg-gray-200"></div>
                            <div className="absolute inset-0 rounded-full" style={{ background: `conic-gradient(#10B981 ${principalPercent}%, transparent ${principalPercent}%)` }}></div>
                            <div className="absolute inset-4 rounded-full bg-white"></div>
                        </div>
                        <div className="w-full mt-6 space-y-3 text-gray-700">
                            <div className="flex justify-between items-center">
                                <span><span className="inline-block w-3 h-3 bg-green-500 rounded-full mr-2"></span>Principal:</span>
                                <span className="font-semibold">{formatCrores(loanAmount)}</span>
                            </div>
                             <div className="flex justify-between items-center">
                                <span><span className="inline-block w-3 h-3 bg-gray-200 rounded-full mr-2"></span>Interest:</span>
                            ...
                            <div className="flex justify-between items-center pt-3 mt-3 border-t border-dashed">
                                ...
                                <span className="text-3xl font-bold text-green-600">{formatLakhs(emi)}</span>
                            </div>
                        </div>
                    </div>
                )}
...
function AmenitiesTabContent({ amenities }: { amenities: Property['amenities'] }) {
    if (!amenities || amenities.length === 0) {
        return <div className="text-center text-gray-500 py-10">No amenities listed for this property.</div>;
    }
