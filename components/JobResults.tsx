      ...
      >
        {options.map((option) => (
          <option key={option} value={option}>
            {option === "All" ? `Select ${label}` : option}
          </option>
        ))}
      </select>
      <ChevronDown
...
const JobFilters = ({
  ...
  onClearFilter: (filterName: keyof Filters) => void;
}) => {
      ...
      {activeFilters.length > 0 && (
        <div className="mt-4 pt-4 border-t border-gray-200 flex flex-wrap items-center gap-2">
          {activeFilters.map(([key, value]) => (
            <div
              key={key}
              className="bg-green-100 text-green-800 text-sm font-medium pl-3 pr-2 py-1 rounded-full flex items-center gap-2"
            >
              <span>{value}</span>
              <button
                onClick={() => onClearFilter(key as keyof Filters)}
      ...
      {open && (
        <div className="mt-6 border-t border-gray-200 pt-6 text-sm">
          ...
          <div className="grid md:grid-cols-2 gap-8 mt-6">
            <DetailSection
              icon={ClipboardList}
              title="Expected Qualifications"
              items={props.qualifications}
            />
            <DetailSection
              icon={ClipboardList}
