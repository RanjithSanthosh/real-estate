...
import { useUI } from '../../app/context/UIContext';
import logoImage from "@/components/homePage/assets/logo.png";
import chatbotIcon from "../../public/assets/chatbot.png";
...
export default function Chatbot() {
  ...
  return (
    <>
      ...
      <div
        className={`fixed bottom-24 right-6 z-50 w-80 md:w-96 bg-white rounded-2xl shadow-2xl border border-gray-200 flex flex-col transition-all duration-300 ease-in-out origin-bottom-right ${
          isChatbotOpen
            ? 'opacity-100 scale-100 pointer-events-auto'
            : 'opacity-0 scale-95 pointer-events-none'
        }`}
          ...
          <div className="flex justify-start">
            <div className="bg-gray-100 rounded-lg p-3 max-w-xs">
              ...
              <p className="text-sm text-gray-700 mt-2">
                Experience truly Award 🏆 winning Service from the Top 3 Best Rated Consultants
                in Chennai & Bengaluru, India.
              </p>
            </div>
          </div>
      ...
      <button
        onClick={toggleChatbot}
        className={`fixed bottom-6 right-6 z-50 w-16 h-16 bg-green-600 rounded-full shadow-2xl flex items-center justify-center transition-all duration-300 ease-in-out hover:scale-110 ${
          isChatbotOpen ? 'opacity-0 scale-0 pointer-events-none' : 'opacity-100 scale-100 pointer-events-auto'
        }`}
      >
        <Image src={chatbotIcon} alt="Chat Icon" width={40} height={40} />
      </button>
