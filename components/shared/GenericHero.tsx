...
//             <motion.button onClick={openOfferModal} whileHover={{ scale: 1.1 }}><Gift size={20} /></motion.button>
//             <motion.button 
//               onClick={isHomePage ? onHeaderSearchClick : () => router.push('/search')}
//               whileHover={{ scale: 1.1 }}
//             >
//               <Search size={20} />
//             </motion.button>
...
// --- Mobile Navigation ---
function MobileNav({ isOpen, onClose, activePage }: { isOpen: boolean; onClose: () => void; activePage: string; }) {
  return (
    <AnimatePresence>
      {isOpen && (
        <motion.div
        ...
        >
          ...
          <nav className="flex flex-col gap-6 text-white text-xl font-medium">
            {NAV_LINKS.map(link => (
              <Link key={link.href} href={link.href} onClick={onClose} className={activePage === link.label ? 'text-green-300' : 'hover:text-green-300'}>
                {link.label}
              </Link>
            ))}
...
interface GenericHeroProps {
  ...
  subtitle: string;
  backgroundImage: StaticImageData | string;
  activePage: 'Home' | 'About' | 'Contact Us' | 'Blog' | 'Careers' | 'Resources';
  showSearchBar?: boolean;
  onSearchClick?: () => void;
  onHeaderSearchClick?: () => void;
  searchTerm?: string;
...
export default function GenericHero({
  title, subtitle, backgroundImage, activePage, showSearchBar = false,
  onSearchClick = () => {}, onHeaderSearchClick = () => {}, searchTerm = '',
  onSearchChange = () => {}, searchBarRef,
}: GenericHeroProps) {
  const router = useRouter();
  const [isMenuOpen, setIsMenuOpen] = useState(false);
  ...
  return (
    <div className={`relative overflow-hidden shadow-lg ${heroHeightClass}`}>
      ...
      <MobileNav isOpen={isMenuOpen} onClose={() => setIsMenuOpen(false)} activePage={activePage} />
        ...
        >
          ...
          <div className="hidden lg:flex items-center gap-8 font-medium">
            <div className="relative" ref={cityDropdownRef}>
              <button onClick={() => handleDropdownToggle('city')} className="flex items-center gap-2">
                {selectedCity}
                <motion.div animate={{ rotate: activeDropdown === 'city' ? 180 : 0 }}><ChevronDown size={16} /></motion.div>
              </button>
              <AnimatePresence>
                {activeDropdown === 'city' && (
                  <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full mt-2 w-48 bg-white text-gray-700 rounded-lg shadow-xl z-40 py-1">
                    {CITIES.map((city) => (
                      <li key={city}><button onClick={() => { setSelectedCity(city); closeAllDropdowns(); }} className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">{city}</button></li>
                    ))}
                  </motion.ul>
                )}
              </AnimatePresence>
            ...
            {NAV_LINKS.map(link => (
              <motion.div key={link.href} whileHover={{ y: -2 }}>
                <Link href={link.href} className={`${activePage === link.label ? 'bg-white text-gray-700' : 'hover:text-green-300'} px-4 py-2 rounded-full transition-colors`}>
                  {link.label}
                </Link>
              </motion.div>
            ))}
            ...
            <div className="relative" ref={resourcesDropdownRef}>
              <button onClick={() => handleDropdownToggle('resources')} className={`flex items-center gap-2 ${activePage === 'Resources' ? 'text-green-300' : 'hover:text-green-300'}`}>
                Resources
                <motion.div animate={{ rotate: activeDropdown === 'resources' ? 180 : 0 }}><ChevronDown size={16} /></motion.div>
              </button>
              <AnimatePresence>
                {activeDropdown === 'resources' && (
                  <motion.ul initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="absolute top-full right-0 mt-2 w-56 bg-white text-gray-700 rounded-lg shadow-xl z-40 py-1">
                    {RESOURCE_LINKS.map((link) => (
                      <li key={link.label}>
                        <Link href={link.href} onClick={closeAllDropdowns} className="block w-full text-left px-4 py-2 text-sm hover:bg-gray-100">{link.label}</Link>
                      </li>
                    ))}
                  </motion.ul>
                )}
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            <motion.button onClick={openConsultationModal} whileHover={{ scale: 1.05 }} className="hidden sm:flex items-center gap-2 border px-4 py-2 rounded-full font-medium"><Phone size={16} /> Call Us</motion.button>
            <motion.button onClick={openOfferModal} whileHover={{ scale: 1.1 }} whileTap={{scale: 0.9}} className="p-2 hover:bg-white/20 rounded-full"><Gift size={20} /></motion.button>
            <motion.button onClick={showSearchBar ? onHeaderSearchClick : () => router.push('/home')} whileHover={{ scale: 1.1 }} whileTap={{scale: 0.9}} className="p-2 hover:bg-white/20 rounded-full"><Search size={20} /></motion.button>
            <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} className="p-2 hover:bg-white/20 rounded-full"><Link href="/favorites"><Heart size={20} /></Link></motion.button>
            <motion.button onClick={openLoginModal} whileHover={{ scale: 1.1 }} whileTap={{scale: 0.9}}><User size={22} /></motion.button>
      ...
      <div className="relative z-20 flex flex-col justify-center h-full text-white px-6 md:px-12">
        <motion.div initial="hidden" animate="visible" variants={{ visible: { transition: { staggerChildren: 0.2 } } }} className="container mx-auto">
          <motion.h1 variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }} className="text-4xl sm:text-5xl md:text-6xl font-bold mb-2">{title}</motion.h1>
          <motion.p variants={{ hidden: { y: 20, opacity: 0 }, visible: { y: 0, opacity: 1 } }} className="text-lg sm:text-xl text-gray-200">{subtitle}</motion.p>
        </motion.div>
      </div>
      ...
      {showSearchBar && (
        <div ref={searchBarRef} className="absolute -bottom-24 md:-bottom-16 left-1/2 -translate-x-1/2 z-20 w-[90%] lg:w-3/4 max-w-4xl">
          <div className="bg-white rounded-2xl md:rounded-full shadow-2xl p-3 flex flex-col md:flex-row items-center gap-2">
            <div className="flex items-center gap-2 w-full md:w-auto md:border-r pr-4 pl-4 py-2 md:py-0"><MapPin className="text-gray-400" size={20} /><span className="font-semibold text-gray-700">{selectedCity}</span></div>
            <div className="relative flex-grow w-full">
              <Search className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={20} />
              <input type="text" placeholder="Search by property name or type..." className="w-full pl-10 pr-4 py-2 bg-transparent border-none focus:ring-0 text-gray-800" value={searchTerm} onChange={(e) => onSearchChange && onSearchChange(e.target.value)} />
            </div>
            <div className="flex items-center gap-2 p-2 md:p-0 w-full md:w-auto justify-evenly">
              <button onClick={openFilterModal} className="p-3 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors"><SlidersHorizontal size={20} /></button>
              <button className="p-3 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors"><Settings2 size={20} /></button>
              <button onClick={onSearchClick} className="p-3 bg-green-500 text-white rounded-full hover:bg-green-600 transition-colors"><Search size={20} /></button>
            </div>
          </div>
        </div>
      )}
