...
const reviews: Review[] = [
  {
    ...
    name: "Priya Sharma",
    rating: 4.5,
    content: "Home Konnect's team was incredibly professional. They understood our needs and found us the perfect apartment faster than we expected.",
  },
  {
    profilePic: 'https://randomuser.me/api/portraits/men/32.jpg',
    name: "Rajesh Kumar",
...
export default function CallToActionAndReviews() {
  ...
  return (
    <section className="bg-white mt-0">
      ...
      >
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center md:items-start justify-between">
          <motion.div variants={containerVariants} className="flex flex-col text-center md:text-left md:w-1/2 z-10">
            <motion.h2 variants={itemVariants} className="text-3xl md:text-4xl font-bold mb-4">
              Have a property requirement ?
            </motion.h2>
            <motion.p variants={itemVariants} className="text-lg mb-8 max-w-md mx-auto md:mx-0">
              Connect with our expert advisors to buy, sell or rent your property.
            </motion.p>
            <motion.div variants={itemVariants}>
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="bg-white text-green-600 font-semibold py-3 px-6 rounded-full transition flex items-center gap-2 justify-center mx-auto md:mx-0">
                Contact Us <ArrowUpRight size={16} />
              </motion.button>
            </motion.div>
          </motion.div>

          <motion.div variants={imageSlideIn} className="hidden md:block absolute right-0 bottom-0 md:w-1/2 lg:w-[40%] h-full">
            <Image
              src={logoImage}
              alt="House in hand"
              fill
      ...
      >
        <div className="container mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12">
          <motion.div variants={itemVariants} className="lg:col-span-4 flex flex-col items-center lg:items-start text-center lg:text-left">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-800 mb-4 max-w-sm">
              What Our Customers Are Saying...
            </h2>
            <p className="text-gray-500 text-lg mb-8">
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="border border-green-400 text-green-600 font-semibold py-2.5 px-6 rounded-full hover:bg-green-50 transition flex items-center gap-2 justify-center">
              Love Us?
            </motion.button>
          </motion.div>

          <motion.div variants={itemVariants} className="lg:col-span-8 flex justify-center">
            <Carousel setApi={setApi} className="w-full max-w-4xl">
              <CarouselContent className="-ml-4">
                {reviews.map((review, index) => (
                  <CarouselItem key={index} className="pl-4 md:basis-1/2">
                    <motion.div whileHover={{ y: -5, scale: 1.02 }} className="bg-white border border-gray-200 p-6 rounded-xl shadow-sm h-full flex flex-col">
                      <div className="flex items-center mb-4">
                        <Image
                          src={review.profilePic}
                          alt={review.name}
