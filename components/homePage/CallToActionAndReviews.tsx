...
export default function CallToActionAndReviews() {
  ...
  const {
    openOfferModal,
  } = useUI(); // ✅ Context hook (make sure provider wraps this component)
  ...
  // ✅ Ensure modal trigger works even inside fixed layers (for mobile Safari / Chrome)
  ...
  return (
    <section className="bg-white mt-0 relative z-10">
      {/* Top Green Banner Section */}
      <motion.div
        initial="hidden"
        whileInView="visible"
        ...
        className="bg-green-600 text-white py-16 md:py-24 relative overflow-hidden"
      >
        <div className="container mx-auto px-6 flex flex-col md:flex-row items-center md:items-start justify-between relative z-20">
          <motion.div
            variants={containerVariants}
            className="flex flex-col text-center md:text-left md:w-1/2 z-20"
          >
            <motion.h2
              variants={itemVariants}
              className="text-3xl md:text-4xl font-bold mb-4"
            ...
            <motion.div variants={itemVariants}>
              <motion.button
                ...
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                className="bg-white text-green-600 font-semibold py-3 px-6 rounded-full transition flex items-center gap-2 justify-center mx-auto md:mx-0 relative z-30 touch-manipulation"
              >
                Contact Us <ArrowUpRight size={16} />
              </motion.button>
            </motion.div>
