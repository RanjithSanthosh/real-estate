...
// --- Property Card ---
const PropertyCard = ({ property, developers }: { property: Property; developers: Developer[] }) => {
  ...
  return (
    <Link href={`/properties/${property.id}`} className="block h-full">
      <motion.div
        ...
        whileHover="hover"
        animate="rest"
        className="bg-white border border-gray-100 rounded-3xl overflow-hidden shadow-md h-full flex flex-col transition-all duration-300 hover:shadow-2xl"
      >
        {/* Image Section */}
        <div className="relative h-56 sm:h-64 w-full overflow-hidden">
          ...
          {property.tag && (
            <div
              className={`absolute top-4 left-4 text-xs font-bold py-1.5 px-3 rounded-full text-white ${
                property.tag.color === 'green'
                  ? 'bg-green-600'
                  : property.tag.color === 'yellow'
                  ? 'bg-yellow-600'
                  : 'bg-blue-600'
              }`}
            >
        ...
        <div className="p-4 sm:p-6 flex flex-col flex-grow">
          ...
          {/* Specs */}
          <div className="flex flex-wrap gap-x-4 gap-y-2 text-gray-600 text-xs sm:text-sm">
            {property.specs.map((spec, index) => (
              <div key={index} className="flex items-center gap-1.5 sm:gap-2">
                <Icon name={spec.icon} size={14} className="text-gray-500" />
                <span>{spec.text}</span>
              </div>
            ))}
          </div>
  ...
  return (
    <div className="flex justify-center items-center gap-4 mt-12">
      <button
        onClick={handlePrev}
        disabled={currentPage === 1}
        className="flex items-center gap-2 px-4 py-2 bg-white border border-gray-300 rounded-full text-sm font-medium text-gray-700 disabled:opacity-50 hover:bg-gray-50"
      >
        <ChevronLeft size={16} />
        Previous
      </button>
  ...
  return (
    ...
    >
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        {propertiesToShow.map((property) => (
          <motion.div key={property.id} variants={gridItemVariants}>
            <PropertyCard property={property} developers={developers} />
          </motion.div>
        ))}
      </div>
