  ...
  <div className="mt-6">
    <div className="relative">
      ...
      <div className="relative flex justify-center text-sm">
        <span className="px-2 bg-white/70 text-gray-600">Or</span>
      </div>
    </div>
    <button className="mt-6 w-full flex items-center justify-center gap-3 py-3 px-4 border border-gray-300 rounded-full shadow-sm bg-white text-sm font-medium text-gray-700 hover:bg-gray-50">
      <Image src={googleIcon} alt="Google Icon" width={20} height={20} />
    ...
    <div className="mt-6 text-center text-sm">
      <p className="text-gray-700">
        ...
        <button
          ...
          className="font-semibold text-green-700 hover:underline ml-1"
        >
          {isLoginView ? "Register Now" : "Login"}
        </button>
      </p>
...
export default function LoginModal({ onClose }: LoginModalProps) {
  ...
  // This effect validates the current form whenever the view or inputs change
  useEffect(() => {
    switch (view) {
      case "LOGIN":
        setIsFormValid(
  ...
  }, [view, loginForm, registerForm, forgotPasswordForm]);

  const handleViewChange = (newView: ModalView) => {
    clearForms(); // Clear all inputs when changing views
    setView(newView);
  };
  ...
  const handleForgotSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!isFormValid) return;
    console.log("Forgot password submitted", forgotPasswordForm);
    setView("LOGIN"); // Go back to login view after "submission"
  };
          ...
          <div className="bg-white/70 backdrop-blur-lg p-8 rounded-2xl md:rounded-l-none md:rounded-r-2xl text-gray-800 overflow-y-auto">
            ...
            {view === "LOGIN" && (
              <>
                <form className="space-y-6" onSubmit={handleLoginSubmit}>
                  <div>
                    ...
                    <input
                      ...
                      autoComplete="email"
                      className="mt-1 block w-full px-4 py-3 bg-white/80 border border-gray-300 rounded-full shadow-sm focus:outline-none focus:ring-green-500 focus:border-green-500"
                  ...
                  <button
                    ...
                    className="w-full bg-green-500 text-white font-bold py-3 px-6 rounded-full text-lg hover:bg-green-600 transition shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Login
                  </button>
                </form>
                <FormFooter
                  isLoginView={true}
                  onToggleView={() => handleViewChange("REGISTER")}
                />
              </>
            )}
            ...
            {view === "REGISTER" && (
              <>
                <form className="space-y-4" onSubmit={handleRegisterSubmit}>
                  <div>
                    ...
                    <input
                      ...
                      autoComplete="name"
                      className="mt-1 block w-full px-4 py-3 bg-white/80 border border-gray-300 rounded-full shadow-sm"
                  ...
                  <button
                    type="submit"
                    disabled={!isFormValid}
                    className="w-full bg-green-500 text-white font-bold py-3 px-6 rounded-full text-lg hover:bg-green-600 transition shadow-lg !mt-6 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    Register
                  </button>
                </form>
                <FormFooter
                  isLoginView={false}
                  onToggleView={() => handleViewChange("LOGIN")}
                />
              </>
            )}
